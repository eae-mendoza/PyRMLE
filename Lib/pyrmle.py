...
class grid_obj:
    ...
    def __init__(self, scale, shifts, interval, dim, step, start, end):
        ...
        self.b0_grid_points = np.array(
            [(self.b0_range[i - 1] + self.b0_range[i]) / 2 for i in range(1, len(self.b0_range))])
        self.b1_grid_points = np.array(
            [(self.b1_range[i - 1] + self.b1_range[i]) / 2 for i in range(1, len(self.b1_range))])
        self.b2_grid_points = np.array(
            [(self.b2_range[i - 1] + self.b2_range[i]) / 2 for i in range(1, len(self.b2_range))])
        self.shifts = shifts
...
def sim_sample3d(n, x_params=None, beta_pi=None, beta_mu=None, beta_cov=None):
    ...
    if beta_cov is not None:
        ...
        beta_cov = [[[0.01, 0, 0], [0, 0.01, 0], [0, 0, 0.01]], [[0.01, 0, 0], [0, 0.01, 0], [0, 0, 0.01]],
...
def cv_loss(a, alphas, n, k, progress, time_start, total, functional, initial_guess, trans_matrix, trans_matrix_long,
            step_size, jacobian, hessian_method, constraints, tolerance, max_iter, bound):
    """ This fucntion is used to compute for the loss function in cross-validation. """
    indices = cv_index(n, k)
    f_n = scop.minimize(functional, initial_guess, args=(a, n, trans_matrix_long, step_size), method='trust-constr',
                        jac=jacobian, hess=hessian_method, constraints=[constraints], tol=tolerance,
                        options={'verbose': False, 'maxiter': max_iter}, bounds=bound)
    fs = []
    trans_matrix_slices = []
    ...
    for t, i in zip(trans_matrix_slices, inv_trans_matrix_slices):
        ...
        n = len(t)
        n_i = len(i)
        val_f = scop.minimize(functional, initial_guess, args=(a, n, trans_matrix_slice_long, step_size),
                              method='trust-constr', jac=jacobian, hess=hessian_method, constraints=[constraints],
                              tol=tolerance, options={'verbose': False, 'maxiter': max_iter}, bounds=bound).x
        fs.append(val_f)
...
def quarter_selector(alphas, n, k, progress, time_start, total, functional, initial_guess, trans_matrix,
                     trans_matrix_long, step_size, jacobian, hessian_method, constraints, tolerance, max_iter, bound):
    ...
    a_p = alphas[p]
    a_q = alphas[q]
    val1 = cv_loss(a_p, alphas, n, k, progress, time_start, total, functional, initial_guess, trans_matrix,
                   trans_matrix_long, step_size, jacobian, hessian_method, constraints, tolerance, max_iter, bound)
    progress_updt = val1[2]
    val2 = cv_loss(a_q, alphas, n, k, progress_updt, time_start, total, functional, initial_guess, trans_matrix,
                   trans_matrix_long, step_size, jacobian, hessian_method, constraints, tolerance, max_iter, bound)
...
def rmle_2d(functional, alpha, tmat, shift=None, k=None, jacobian=None, initial_guess=None, hessian_method=None,
            constraints=None, tolerance=None, max_iter=None, bounds=None):
...
Try to supply any of the accepted functional values: {likelihood, norm_sq, sobolev, entropy}")
    ...
    if any(c in str.lower(str(alpha)) for c in lepskii_matches):
        ...
        for a in alphas:
            rec = scop.minimize(functional, initial_guess, args=(a, n, trans_matrix_long, step_size),
                                method='trust-constr', jac=jacobian, hess=hessian_method, constraints=[constraints],
                                tol=tolerance, options={'verbose': False, 'maxiter': max_iter}, bounds=bound)
    ...
    if any(c in str.lower(str(alpha)) for c in cv_matches):
        ...
        while len(alphas) > 4:
            val = quarter_selector(alphas, n, k, j, time_start, total, functional, initial_guess, trans_matrix,
                                   trans_matrix_long, step_size, jacobian, hessian_method, constraints, tolerance,
                                   max_iter, bound)
            lhood.append(val[2])
        ...
        for a in alphas:
            val = cv_loss(a, alphas, n, k, j, time_start, total, functional, initial_guess, trans_matrix,
                          trans_matrix_long, step_size, jacobian, hessian_method, constraints, tolerance, max_iter,
                          bound)
            lhood.append(val[1])
            reconstructions.append(val[0])
            alpha_list.append(a)
        ...
        result = scop.minimize(functional, initial_guess, args=(alpha, n, trans_matrix_long, step_size),
                               method='trust-constr', jac=jacobian, hess=hessian_method, constraints=[constraints],
                               tol=tolerance, options={'verbose': 0, 'maxiter': max_iter}, bounds=bound)
        et = time.time()
        details = {
            "Regularization Functional": str(functional.__name__),
            "Jacobian": str(jacobian.__name__),
...
def get_box_index(point, start, end, step_size, interval, ks):
    ...
    if point_check(point, start, end) == 3:  # case for outer-vertex
        ...
        if (abs(point[1]) - abs(start) == 0 and abs(point[2]) - abs(start) == 0) or (
                abs(point[1]) - abs(end) == 0 and abs(point[2]) - abs(end) == 0):
        ...
        elif (abs(point[0]) - abs(start) == 0 and abs(point[2]) - abs(start) == 0) or (
                abs(point[0]) - abs(end) == 0 and abs(point[2]) - abs(end) == 0):
            indices = np.array([get_index(point, interval, ks[2])] * 2, dtype=int)
            adjustments = np.array([0, ks[0]], dtype=int)
            return list(indices + adjustments)

        elif (abs(point[0]) - abs(start) == 0 and abs(point[1]) - abs(start) == 0) or (
                abs(point[0]) - abs(end) == 0 and abs(point[1]) - abs(end) == 0):
            indices = np.array([get_index(point, interval, ks[2])] * 2, dtype=int)
            adjustments = np.array([0, ks[1]], dtype=int)
            return list(indices + adjustments)

    elif ((sum((abs(point) - abs(start)) == 0) == 1 or sum((abs(point) - abs(end)) == 0) == 1) and vertex_check(point,
                                                                                                                interval) == 3):
...
def rmle_3d(functional, alpha, tmat, shift=None, k=None, jacobian=None, initial_guess=None, hessian_method=None,
            constraints=None, tolerance=None, max_iter=None, bounds=None):
    """ This function is a sub-function of rmle(). This is a wrapper function for the SciPy Optimize minimize()
    function. It has 3 essential arguments which are: {functional, alpha, tmat}.
...
Try to supply any of the accepted functional values: {likelihood_3d, norm_sq_3d, sobolev_3d, entropy_3d}")
    ...
    if shift == True:
        ...
        while i < num_shifts:
            ...
            shifted_grid = grid_obj(interval=tmat.grid.interval, scale=tmat.grid.scale, shifts=shifts,
                                    dim=tmat.grid.dim, step=step_size, start=tmat.grid.start, end=tmat.grid.end)
            ...
            rec = scop.minimize(functional, initial_guess, args=(alpha, n, shift_trans_matrix_long, step_size),
                                method='trust-constr', jac=jacobian, hess=hessian_method, constraints=[constraints],
                                tol=tolerance, options={'verbose': 0, 'maxiter': max_iter}, bounds=bound)
            reconstructions.append(rec.x)
            i += 1
...
def scale_sample(xy_sample, grid):
    ...
    if grid.dim == 2:
        ...
        sample[:, 3] = sample[:, 3] + grid.shifts[0] * sample[:, 0] + grid.shifts[1] * sample[:, 1] + grid.shifts[
            2] * sample[:, 2]
        sample[:, 0] = sample[:, 0] / grid.b0
        sample[:, 1] = sample[:, 1] / grid.b1
